...
export async function reportesRoutes(app: FastifyInstance) {
    app.get("/api/reportes", async (req, reply) => {
        ...
        if (q.codigo && q.codigo.trim() !== "") {
            filters.codigo = q.codigo;
        }
        if (q.genero && q.genero.trim() !== "") {
            filters.genero = q.genero;
        }
        if (q.categoria && q.categoria.trim() !== "") {
            filters.categoria = q.categoria;
        }
        if (q.subcategoria && q.subcategoria.trim() !== "") {
            filters.subcategoria = q.subcategoria;
        }
