...
# -*- coding: utf-8 -*-
"""
ml_proyeccion_optimizada_xgboost.py

Predicción de ventas con XGBOOST + Cross Validation + Hyperparameter Tuning.
MEJORAS APLICADAS:
1. Modelo: Cambio de Random Forest a XGBoost (Mayor precisión).
2. Limpieza de texto (eliminación de dígitos y nulos).
3. Transformación Logarítmica + Factor de Ajuste de Volumen.
"""
...
# --- CAMBIO IMPORTANTE: Importamos XGBoost ---
...
class Config:
    ...
    min_datos_entrenamiento: int = 12  # Recomendado subir a 12 para XGBoost
...
def leer_datos(cfg: Config) -> pd.DataFrame:
    ...
    # ---iguala los string a mayusculas ---
...
# ---------------- MOTOR DE MACHINE LEARNING (XGBOOST) ----------------

def entrenar_optimizar_predecir(df: pd.DataFrame, cfg: Config):
    """
    Recorre cada grupo, optimiza hiperparámetros XGBoost y aplica corrección de volumen.
    """
    resultados_total = []
    resultados_detalle = []
    metricas_modelo = []
    ...
    print(f"🚀 Iniciando XGBoost en {total_grupos} grupos...")
    ...
    for (genero, cat, subcat), grupo in grupos:
        ...
        # Usamos objective='reg:squarederror' que es estándar para regresión
        modelo_base = XGBRegressor(objective='reg:squarederror', n_jobs=1, random_state=42)
        
        # Grilla específica para XGBoost
        param_grid = {
            "n_estimators": [100, 200, 300],
            "max_depth": [3, 5, 7],      # Árboles más profundos pueden sobreajustar
            "learning_rate": [0.01, 0.05, 0.1, 0.2], # 'eta': Velocidad de aprendizaje
            "subsample": [0.7, 0.8, 1.0], # Evita overfitting tomando muestras parciales
        ...
        try:
            ...
            # Fallback simple
            best_model = XGBRegressor(n_estimators=100, objective='reg:squarederror', random_state=42)
            best_model.fit(X, y)

        # 4. Cálculo de Métricas y FACTOR DE AJUSTE DE VOLUMEN
        # Predicción histórica en Log
        preds_log = best_model.predict(X)
        ...
        # Límite de seguridad (para no multiplicar por 10 si el error es gigante)
...
def guardar_resultados(df_total, df_detalle, df_metricas, cfg: Config):
    ...
    if not df_metricas.empty:
        archivo_metricas = "reporte_metricas_xgboost.csv"
