...
# -----------------------------------
server_sqlserver = '192.168.1.131,1434'  
database_sqlserver = 'INTERCO2013'  
username_sqlserver = 'INTERCO2013'
...
query = """
SELECT
    LEFT(KOPR, 4) + '_' + SUBSTRING(KOPR, 5, 2) AS codigo_color,
    SUM(STFI1) AS stock
FROM dbo.MAEST
GROUP BY
    LEFT(KOPR, 4) + '_' + SUBSTRING(KOPR, 5, 2);
"""
...
with engine_mysql.connect() as conn:
    conn.execute(text("TRUNCATE TABLE stock_interco;"))
    conn.commit()

df.to_sql('stock_interco', con=engine_mysql, if_exists='append', index=False)
