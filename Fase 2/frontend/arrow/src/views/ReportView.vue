...
/* Columnas (nota: format recibe (v,row)) */
const columns = [
  ...
  {
    key: "articulos_en_linea",
    label: "Artículos en línea",
    align: "right" as const,
    format: (v: unknown) => fmt(v, 0),
  },
  {
    key: "ventas_prom_6m_estimado",
    label: "Ventas prom 6m",
    align: "right" as const,
    format: (v: unknown) => fmt(v, 2),
  },
  {
    key: "ventas_prom_x_articulo_stimado",
    label: "Ventas prom x art.",
    align: "right" as const,
    format: (v: unknown) => fmt(v, 2),
  },
  {
    key: "fecha_proyeccion",
    label: "Fecha proyección",
    align: "left" as const,
    format: (v: unknown) => fmt(v),
  },
];
</script>
...
<template>
  ...
  <!-- DataTable espera :rows y permite :rowKey opcional -->
